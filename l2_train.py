...
parser.add_argument('--name', default='TripletNet-default', type=str,
...
def train(train_loader, net, criterion, optimizer, epoch):
    ...
    for batch_idx, (anchor, positive, negative) in enumerate(train_loader):
        ...
        loss_triplet = criterion(embedded_anchor, embedded_positive, embedded_negative)
        loss_norm = embedded_anchor.norm(2) + embedded_positive.norm(2) + embedded_negative.norm(2)
        loss = loss_triplet + 0.0001 * loss_norm

        # measure accuracy and record loss
        acc = accuracy(embedded_anchor, embedded_positive, embedded_negative)
...
def test(test_loader, net, criterion, epoch):
    ...
    for batch_idx, (anchor, positive, negative) in enumerate(test_loader):
        ...
        loss_triplet = criterion(embedded_anchor, embedded_positive, embedded_negative)
        loss_norm = embedded_anchor.norm(2) + embedded_positive.norm(2) + embedded_negative.norm(2)
        test_loss = loss_triplet# + 0.001 * loss_norm
...
def accuracy(emb_anc, emb_pos, emb_neg):
    pdist = nn.PairwiseDistance(p=2)
    dista = pdist(emb_anc, emb_pos)
    distb = pdist(emb_anc, emb_neg)
    pred = (dista - distb + args.margin).cpu().data
